...
CACHE_FREQUENCY = 100
CACHE_FILE = "classified_data.jsonl"
...
dataset = load_dataset("teknium/OpenHermes-2.5", split='train', keep_in_memory=False)
...
progress_bar = tqdm(total=total_samples, desc="Processing", unit="sample", initial=start_index)
...
try:
    ...
    for i, sample in enumerate(samples, start=start_index):
        ...
        print(f"Processed item {i+1}/{total_samples}: Category: {item['tech_category']}, Score: {item['classification_score']:.2f}")
    ...
    if batch:
        save_to_cache(batch)
        classified_data.extend(batch)
    print(f"Progress saved. Restart the script to continue from sample {len(classified_data)}")
    exit(0)
